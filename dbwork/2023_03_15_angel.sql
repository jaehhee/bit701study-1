...
--제약조건을 제거  : name 에 not null 제약조건이 들어있다(제약조건이름을 안줘서 기본 이름으로 들어가있다)
--SYS_C008317 제거
alter table person drop CONSTRAINT SYS_C008317;

--PPT 30 페이지
--NAME 에 새로운 제약조건을 추가해보자(UNIQUE)
...
--AGE 에10~50 세만 추가 가능하도록 제약조건을 추가해보자
...
--문제2
--BUSEO 라는 컬럼을 추가VARCHAR2(10) 기본값을 '교육부'
...
--문제3
--강씨,이씨성을 가진 사람들은 모두 '인사부' 로 데이타수정
...
select buseo 부서명,count(*) 인원수,round(avg(age),0) 평균나이 from person group by buseo order by 부서명;

--시퀀스 생성하는 방법 ppt 78페이지
