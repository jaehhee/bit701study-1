-- sawon 에 제약조건 추가하기
-- score 는 0~100 만 추가가능하도록 제약조건 추가
...
-- join 연습할 서브 테이블 생성  sawoninfo  idx,num(외부키),addr,hp,age 컬럼 - 부모테이블 데이타 삭제시 
-- 서브테이블 데이타 자동 삭제하려면 제약조건 뒤에 on delete cascade 추가
...
-- sawoninfo 에 데이타 추가(반드시 sawon 의 num 에 있는값만 넣을수 있다)
...
-- on delete cascade : sawon  에서 2번 데이타 삭제시 sawoninfo 에서도 삭제됨
-- 깜박잊고 안줫음
-- 문제:기존 외부키 제약조건 제거하고 다시 주세요(이번에는 on delete cascade 추가)
...
select lcase('Happy'),lower('Happy') from dual;-- 두개 다 소문자 변환
...
select greatest(12,34,56,100,66) from dual; -- 가장 큰숫자
